...
class Chat:
    ...
    async def chat_with_all(self, completions_args):
        ...
        param['messages'] = query
        logs = f"ChatGPT request param: \n{json.dumps(param, ensure_ascii=False, indent=None)}\n"
                    ...
                    if response.status == 200:
                        ...
                        if response.content_type == 'application/json':
                            ...
                            if isinstance(answer, list):
                                answer = next((item['text']['content'] for item in answer if item.get('type') == 'text'), answer)
                            ...
                            async for line in response.content:
                                ...
                                if json_string:  # 检查内容是否为空
                                    try:
                                        ...
                                        code = -1
                                        messages = f"{messages}, JSONDecodeError, ChatGPT Data Invalid JSON: {json_string}. "
                                        api_logger.error(messages)
                        else:
                            code = -1
                        ...
                        code = -1
                        messages = f'ChatGPT response failed with status code: {response.status}. '
    ...
    def update_user_info(self, settings_args):
        ...
        if settings_args['channel'] not in ["FastGPT", "Langchain"]:
            code = 1
            messages = "Unsupported chat mode. Please use 'FastGPT' or 'Langchain'."
            return code, messages
        elif settings_args['mode'] not in ["direct", "knowledge_base"]:
            code = 1
            messages = "Unsupported chat channel. Please use 'direct' or 'knowledge_base'."
            return code, messages
